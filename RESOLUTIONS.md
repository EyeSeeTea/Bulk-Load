...
`package.json` can record *what* is constrained but not *why*, so without this file the block grows
...
> **Ranges, not exact versions.** Almost every entry below is a *floor* — "never below this" — so it
> takes a compatible range. An exact version is a *fixture*, used only when something binds to that
...
- **Why:** `@eyeseetea/d2-api@1.21.0` requests `axios@1.6.4` as an exact version, so removing this
  resolution resolves `axios` *downwards* to 1.6.4 rather than upwards. The floor is required for as
...
- **Why:** `@eyeseetea/d2-api@1.21.0` and `@eyeseetea/d2-ui-components@2.12.0` both request
  `lodash@4.17.21` as an exact version. Removing the resolution resolves *downwards* to 4.17.21,
...
- **Drop when:** `i18next-conv` requests `node-gettext@^3` or later, or the `@dhis2/d2-i18n-*` pair is
  replaced (see *Dependency decisions with no trace in the manifest*).
...
entry above shows, removing a resolution can resolve a package *downwards*.
...
The trade is therefore not *abandoned → maintained*. It is *two frozen packages that currently
require no constraint* in exchange for *a maintained package that carries an abandoned chain with no
possible fix*, plus several scoped resolutions to offset the rest of what it brings in. Both
