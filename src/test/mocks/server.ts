...
export function initializeMockServer() {
    ...
    mock.onGet("/dataSets", {
        params: {
            paging: false,
            fields: "access,attributeValues[attribute[code],value],dataSetElements[dataElement[categoryCombo[id],formName,id,name,optionSet[id,options[code,id]],valueType]],displayName,id,name,periodType,sections[dataElements[categoryCombo[id],formName,id,name,optionSet[id,options[code,id]],valueType],id,name]",
            filter: [],
        },
    }).reply(200, {
        dataSets: [
